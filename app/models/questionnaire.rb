class Questionnaire < ActiveRecord::Base
  ...
  def create_new_questionnaire_obj(params)
  ...
  private
    ...
    def create_questionnaire_node
      tree_folder = TreeFolder.where(['name like ?', self.display_type]).first
      parent = FolderNode.find_by(node_object_id: tree_folder.id)
      QuestionnaireNode.create(parent_id: parent.id, node_object_id: self.id, type: 'QuestionnaireNode')
    end
